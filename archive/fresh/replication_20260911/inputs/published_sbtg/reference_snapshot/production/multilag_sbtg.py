...
# Broadened HP ranges for better exploration (Jan 2026)
DEFAULT_HP_NOISE_STD_RANGE = (0.01, 0.5)  # Was (0.05, 0.3), now wider
DEFAULT_HP_HIDDEN_DIM_CHOICES = [32, 64, 128, 256]  # Added 256
DEFAULT_HP_LR_RANGE = (5e-5, 5e-2)  # Was (1e-4, 1e-2), now ~2x wider on each end
DEFAULT_HP_N_STARTUP_TRIALS = 20  # Random trials before Bayesian optimization
...
class MultiBlockSBTGEstimator:
    ...
    def _build_multiblock_windows(
        ...
        for u, X in enumerate(X_list):
            ...
            for t in range(p, T):
                # Window: z = (x_{t-p+1}, x_{t-p+2}, ..., x_t, x_{t+1})
                # At position t, x_{t+1} = X[t] (we're predicting "next")
                # But we need to think of t as the "present" timestep
                
                # Actually: z_t = (x_{t-p+1}, ..., x_t, x_{t+1})
                # If we're at index t in the loop:
                #   x_{t-p+1} = X[t-p]
                #   x_{t-p+2} = X[t-p+1]
                #   ...
                #   x_t = X[t-1]  
                ...
                # So blocks are: X[t-p], X[t-p+1], ..., X[t-1], X[t]
