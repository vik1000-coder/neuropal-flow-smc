...
def load_imputed_data() -> Tuple[List[np.ndarray], List[str]]:
    ...
    # Load segments
    X_list = load_trace_collection(data_dir / "traces.npz")
...
def sample_hp_configurations(n_configs: int, seed: int = 42, device: Optional[str] = None) -> List[Dict]:
    ...
    Random sampling (rather than optimization) measures correlation across the HP space.
    """
    np.random.seed(seed)
...
def align_and_evaluate(
    ...
    """
    Align neurons between a prediction and a biological reference, then compute metrics.
    
    Args:
        pred_adj: Predicted adjacency matrix (model output)
        pred_neurons: Neuron names for prediction
        true_adj: Reference adjacency (or q-values)
        true_neurons: Neuron names for the reference
...
def main():
    ...
    for _, row in corr_df.iterrows():
        ...
        # Interpretation: higher is better for every candidate objective.
    ...
    # Best by Cook-reference AUROC
    if len(valid_df) > 0:
        best_cook = valid_df.loc[valid_df['cook_auroc'].idxmax()]
        print(f"    Cook-reference AUROC (highest): trial {int(best_cook['trial'])}, "
