...
whether multi-lag SBTG functional connectivity predictions align better
...
Scope:
    The analysis compares descriptive alignment across the tested lags. It
    does not identify receptor kinetics, biological response times, or causal
    effects.
...
BENTLEY_DIR = PROJECT_ROOT / "reference_data" / "modulatory_atlas"
...
def normalize_bentley_name(name: str) -> str:
    """
    Normalize a Bentley neuron name to match the model naming convention.
    
    The analysis consumes Bentley's class-level edge list, whose identifiers
    already match the model's cell-class convention.
    
    Args:
        name: Raw neuron name from Bentley data
        
    Returns:
        Normalized name matching the model convention
    """
...
def load_model_neuron_names(dataset: str = "full_traces_imputed") -> List[str]:
    """Load model neuron names from prepared data or the released paper artifact."""
    ...
    paper_path = PROJECT_ROOT / "results" / "paper" / "sbtg_lag_matrices.npz"
    if dataset == "full_traces_imputed" and paper_path.is_file():
        with np.load(paper_path, allow_pickle=False) as archive:
            return [str(value) for value in archive["neuron_names"]]

    raise FileNotFoundError(
        f"Could not find prepared neuron order at {std_path} or a compatible "
        f"released artifact at {paper_path}"
    )
...
class MonoamineNetwork:
    ...
    adjacency: np.ndarray   # (n, n) binary adjacency aligned to model neurons
    neuron_names: List[str] # Model neuron names
    n_raw_edges: int        # Total edges before filtering
    n_filtered_edges: int   # Edges surviving the model-neuron filter
...
def load_monoamine_edges(include_dop56: bool = False) -> pd.DataFrame:
    ...
    """
    if include_dop56:
        csv_path = BENTLEY_DIR / "edge_lists" / "edgelist_MA_classes_incl_dop-5_dop-6.csv"
    else:
        csv_path = BENTLEY_DIR / "edge_lists" / "edgelist_MA_classes.csv"
...
def build_monoamine_adjacency(
    ...
    Args:
        ...
        neuron_names: Model neuron names (determines matrix order)
    ...
    Returns:
        ...
        n_filtered_edges: Unique directed edges where both endpoints occur in
            neuron_names
    """
    n = len(neuron_names)
    ...
    n_filtered_edges = int(np.count_nonzero(adjacency))
...
def load_all_monoamine_networks(
    ...
    Args:
        neuron_names: Model neuron names
...
def validate_monoamine_data(
    ...
    Expected row counts in the bundled class-level edge list are checked before
    duplicate receptor rows are collapsed into unique directed class edges.
    """
    print("\n" + "="*60)
    print("MONOAMINE NETWORK VALIDATION")
    print("="*60)
    
    # Expected raw counts from the reference dataset.
    expected = {
        'dopamine': 255 if include_dop56 else 159,
        'serotonin': 120,
        'tyramine': 54,
        'octopamine': 15,
    }
    ...
    total_raw = 0
    total_filtered_by_transmitter = 0
    ...
    for trans in ['dopamine', 'serotonin', 'tyramine', 'octopamine']:
        ...
        print(f"  {trans:12s}: {net.n_raw_edges:4d} raw {match} (expected {exp}), "
              f"{net.n_filtered_edges:3d} in model neurons ({retention:.1f}%)")
        
        total_raw += net.n_raw_edges
        total_filtered_by_transmitter += net.n_filtered_edges
    
    print(
        f"  {'TOTAL':12s}: {total_raw:4d} raw class rows, "
        f"{total_filtered_by_transmitter:3d} unique edges summed by transmitter; "
        f"{networks['all'].n_filtered_edges:3d} unique after combining transmitters"
    )
    ...
    # Known monoamine sources represented in the model data.
    known_sources = {
        'ADE': 'dopamine',
        'CEP': 'dopamine', 
        'ADF': 'serotonin',
    ...
    for neuron, trans in known_sources.items():
        if neuron in name_to_idx:
            ...
            print(f"  {neuron:4s} ({trans:10s}): NOT IN MODEL NEURONS")
...
def compute_auroc(prediction: np.ndarray, ground_truth: np.ndarray) -> float:
    """
    Compute AUROC between prediction scores and a binary reference network.
...
def main():
    ...
    # Load the model neuron names.
    try:
        neuron_names = load_model_neuron_names(args.dataset)
    ...
    if args.validate:
        validate_monoamine_data(networks, include_dop56=args.include_dop56)
