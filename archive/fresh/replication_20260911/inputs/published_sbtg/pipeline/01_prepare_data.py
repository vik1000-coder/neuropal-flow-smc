...
REQUIREMENTS:
    ...
    - data/Tail_Activity_OH16230.mat       (tail recordings; omit with --no-tail)
...
from pipeline.utils.io import save_trace_collection
...
def load_neuropal_data(
    ...
    if collapse_dv:
        ...
        # Keep the original traces and indices, but expose the collapsed names
        # and a mapping mechanism. The existing structure does not replace
        # "neuron_names" with a unique list yet, because other functions rely on
        # 1-to-1 mapping with traces.
        # The consuming functions handle the grouping.
        # Validity requires a unique neuron list for downstream pipeline stages.
...
def build_donor_map(
    ...
    Args:
        selected_nodes: List of neurons to include
        neuropal_data: NeuroPAL recording data
...
def build_full_trace_dataset_with_imputation(
    ...
    For worms missing certain neurons, copy traces from donor worms that
    have those neurons. This permits use of all 21 worms instead of just
    ...
    save_trace_collection(stim_dir / "traces.npz", time_series)
...
def main():
    ...
    # Locate project root
    try:
        project_root = locate_project_root(Path(__file__).resolve().parent.parent)
...
Data Expansion Options:
  ...
  The OH15500 recordings are not supported by this preprocessing command.
"""
    )
    parser.add_argument(
        "--full-traces", 
