...
def validate_wormatlas(results: List[ClassificationResult], 
    ...
    for r in results:
        ...
        if expected == 'Unknown' or r.classification == 'Insufficient':
            ...
            match = 'N/A'  # The molecular reference label is ambiguous
        else:
            match = 'Mismatch'
        
        rows.append({
            'neuron': name,
            'sbtg_classification': r.classification,
...
def plot_validation_comparison(validation_df: pd.DataFrame, output_dir: Path):
    ...
    known = known[known['sbtg_classification'].isin(['Excitatory', 'Inhibitory', 'Ambiguous'])]
    ...
    for _, row in known.iterrows():
        ...
        pred_idx = predicted_cats.index(row['sbtg_classification'])
    ...
    ax.set_xlabel('SBTG Classification', fontsize=12)
...
For each neuron i, count the number of outgoing excitatory (n₊) and inhibitory (n₋) edges,
...
For each neuron, compute the sum of signed edge weights (E/I score) and use
...
Model the probability θᵢ of neuron i producing excitatory edges as a latent
...
Compare SBTG functional E/I classifications with curated neurotransmitter types
from the WormAtlas database and published C. elegans neuroscience literature.

## Molecular Reference Sources
...
**Accuracy:** Fraction of neurons where the SBTG classification matches the expected functional type
...
**Match:** The SBTG functional classification agrees with the neurotransmitter-based expectation
...
- **Molecular reference:** Uses curated molecular identity
...
**Method:** Compare SBTG functional classifications with curated neurotransmitter types 
from WormAtlas and published literature (Pereira et al. 2015, Bentley et al. 2016).

| Aspect | Details |
|--------|---------|
| **Molecular Reference** | GABAergic → Inhibitory; Glutamatergic/Cholinergic → Excitatory |
...
| Validating against molecular annotations | **E. WormAtlas** |
...
2. The dataset has predominantly excitatory functional effects
3. Neurons covered by the dataset are mostly excitatory types
