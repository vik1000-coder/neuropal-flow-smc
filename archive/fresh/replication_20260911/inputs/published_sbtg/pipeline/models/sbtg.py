...
def gaussian_smooth_1d(
    ...
    if causal:
        ...
        for j in range(n):
            ...
            # Flip the kernel, use 'full' mode, then slice.
...
class SBTGStructuredVolatilityEstimator:
    ...
    def __init__(
        ...
        # Window length is always 2 (x_t and x_{t+lag}); time_lag is configurable.
        if window_length != 2:
    ...
    def _make_train_mask(self, stim_ids: np.ndarray, local_t: np.ndarray, N_total: int) -> np.ndarray:
        ...
        choose to restrict it; this option controls training selection and standardization stats.
        """
    ...
    def _create_fold_assignments(
        ...
        Folds are assigned in a strided manner within each stimulus segment to ensure
