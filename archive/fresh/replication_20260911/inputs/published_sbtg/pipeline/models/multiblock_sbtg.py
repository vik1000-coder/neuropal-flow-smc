...
def compute_validation_loss(
    ...
    with torch.no_grad():
        for (batch,) in loader:
            # Validation uses the same noise level without backpropagation.
...
def compute_validation_loss_structured(
    ...
    Note: torch.no_grad() cannot be used because the structured model's forward()
    method uses torch.autograd.grad() to compute the score from the energy.
    Tensors are detached after computing loss to avoid memory buildup.
    """
    model.eval()
...
def extract_structured_block_scores(
    ...
    torch.no_grad() cannot be used, so evaluation proceeds in batches.
...
def multiblock_mean_test_per_lag(
    ...
    # Compute this for each (j, i) pair.
