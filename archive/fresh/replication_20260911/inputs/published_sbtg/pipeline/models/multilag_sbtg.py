...
def _derive_torch_seed(random_state: int, *components: int) -> int:
    """Derive a stable per-task Torch seed from an estimator seed."""
    seed_sequence = np.random.SeedSequence(
        [int(random_state), *(int(component) for component in components)]
    )
    return int(seed_sequence.generate_state(1, dtype=np.uint32)[0])


def _build_model_with_seed(
    model_factory: Callable[[], nn.Module],
    random_state: Optional[int],
) -> nn.Module:
    """Construct a model deterministically without advancing the caller's RNG."""
    if random_state is None:
        return model_factory()

    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(_derive_torch_seed(random_state, 0))
        return model_factory()
...
def train_score_model(
    ...
    When ``random_state`` is provided, independent local generators control
    DataLoader shuffling and DSM noise. The caller remains responsible for
    seeded model construction (see ``_build_model_with_seed``).
    """
    ...
    if random_state is not None:
        ...
        loader_generator.manual_seed(_derive_torch_seed(random_state, 1))

        dsm_generator = torch.Generator(device=train_tensor.device)
        dsm_generator.manual_seed(_derive_torch_seed(random_state, 2))
    ...
    for epoch in range(epochs):
        ...
        for (batch,) in loader:
            ...
            loss = dsm_loss(model, batch, noise_std, generator=dsm_generator)
...
def compute_null_contrast(
    ...
    # The null uses a shuffled version of the same matrix, approximating
    # results from temporally shuffled data.
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        try:
            ...
            null_contrast = _evaluate_config(
                ...
                n_folds=n_folds,
                device=device,
                random_state=_derive_torch_seed(seed, trial.number),
            )
...
def _evaluate_config(
    ...
    for fold_k in range(n_folds):
        fold_seed = _derive_torch_seed(
            random_state, 0, lag, n_blocks, fold_k
        )
        ...
        # Create model
        if n_blocks == 2:
            model = _build_model_with_seed(
                lambda: TwoBlockStructuredScoreNet(
                    n_neurons=n_neurons,
            ...
            model = _build_model_with_seed(
                lambda: MultiBlockStructuredScoreNet(
                    n_neurons=n_neurons,
                    p_max=n_blocks - 1,
                    hidden_dim=config.hidden_dim,
                    num_layers=config.num_layers,
        ...
        train_score_model(
            ...
            device, verbose=False, random_state=fold_seed,
        )
...
class PerLagSBTGEstimator:
    ...
    def _build_windows_for_lag(
        ...
        for u, X in enumerate(X_list):
            ...
            for t in range(lag_r, T):
                ...
                # Future block: x_{t+1} 
                # At position t, future = x[t] and past = x[t - lag_r].
    ...
    def _cross_fit(
        ...
        for fold_k in range(self.n_folds):
            fold_seed = _derive_torch_seed(
                self.random_state, 1, lag_r, fold_k
            )
            if self.verbose:
            ...
            train_score_model(
                ...
                self.device, verbose=False, random_state=fold_seed,
            )
...
class MultiBlockSBTGEstimator:
    ...
    def _cross_fit(
        ...
        for fold_k in range(self.n_folds):
            fold_seed = _derive_torch_seed(
                self.random_state, 2, self.p_max, fold_k
            )
            if self.verbose:
                print(f"  Fold {fold_k + 1}/{self.n_folds}...", flush=True)
...
class MinimalMultiBlockEstimator:
    ...
    def _cross_fit_lag(
        ...
        for fold_k in range(self.n_folds):
            fold_seed = _derive_torch_seed(
                self.random_state, 3, lag, fold_k
            )
            if self.verbose:
                print(f"    Fold {fold_k + 1}/{self.n_folds}...", flush=True)
