...
def create_derivative_phases(
    ...
    # If too few peaks are found, use uniform phases as a fallback.
...
def load_structural_connectome() -> Tuple[np.ndarray, List[str]]:
    ...
    A_struct = np.load(CONNECTOME_DIR / "A_struct.npy", allow_pickle=False)
...
def create_sbtg_hp_objective(X_train: List[np.ndarray], X_val: List[np.ndarray]):
    ...
    def objective(trial):
        ...
        try:
            ...
            # Return negative null_contrast because Optuna minimizes this objective.
...
def run_sbtg_hp_search(
    ...
    best_null_contrast = -study.best_trial.value  # Convert the minimized negative value back to positive.
