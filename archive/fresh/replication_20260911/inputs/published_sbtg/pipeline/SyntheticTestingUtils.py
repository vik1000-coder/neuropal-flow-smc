...
* **Linear VAR(2)** (family="var") – ``x_t = A1 x_{t-1} + A2 x_{t-2} + eps``.
  Lag matrices are jointly rescaled, when necessary, so the spectral radius of
  the VAR(2) companion matrix is below one.
...
def companion_spectral_radius(A1: np.ndarray, A2: np.ndarray) -> float:
    """Return the spectral radius of a VAR(2) companion matrix.

    Stability of ``x_t = A1 x_{t-1} + A2 x_{t-2} + eps_t`` depends on the
    joint companion matrix ``[[A1, A2], [I, 0]]``.  Checking the two lag
    matrices separately is not sufficient.
    """
    A1 = np.asarray(A1, dtype=float)
    A2 = np.asarray(A2, dtype=float)
    if A1.ndim != 2 or A1.shape[0] != A1.shape[1] or A2.shape != A1.shape:
        raise ValueError("A1 and A2 must be square matrices with identical shapes")

    n = A1.shape[0]
    companion = np.block(
        [[A1, A2], [np.eye(n, dtype=float), np.zeros((n, n), dtype=float)]]
    )
...
def stabilize_var(
    ...
    """Make a VAR(2) stable while preserving coefficient support and signs.

    If the companion radius is ``rho > target``, this applies
    ``A1 <- s A1`` and ``A2 <- s^2 A2`` with ``s = target / rho``.
    The transformation scales every companion root by ``s`` and therefore
    places the joint spectral radius at ``target`` without adding or
    removing edges.
    """
    ...
    A1 = np.asarray(A1, dtype=float)
    A2 = np.asarray(A2, dtype=float)
    radius = companion_spectral_radius(A1, A2)
    if radius == 0.0 or radius <= target:
        return A1.copy(), A2.copy()

    scale = target / radius
    return A1 * scale, A2 * (scale ** 2)
...
def generate_var_data(
    ...
    """
    Generate stationary VAR(2) data with ground-truth adjacency.
    ...
    truth_dict = {1: (A1 != 0.0), 2: (A2 != 0.0)}
    A1, A2 = stabilize_var(A1, A2, target=0.9)
...
def hyperparam_search_sbtg_null_contrast(
    ...
    correlates better with biological reference networks than F1 on synthetic data.
