...
from pipeline.utils.io import load_trace_collection
...
def load_prepared_dataset(stimulus: str = "nacl") -> Tuple[List[np.ndarray], List[str]]:
    ...
    # Load segments
    traces_file = stimulus_dir / "traces.npz"
    if not traces_file.exists():
        raise FileNotFoundError(f"traces.npz not found in {stimulus_dir}")
    X_segments = load_trace_collection(traces_file)
...
def compute_validation_dsm_loss(
    ...
    No Cook or functional-atlas reference labels are used.
    ...
    def objective(trial):
        ...
        if extended:
            ...
            params = {
                ...
                # Avoid the overfitting observed below an L1 penalty of 5e-4.
        ...
        params.update({
            ...
            'train_frac': 0.8,
            'verbose': False,
            # Use in-sample training for this hyperparameter-search objective.
            # 1. Train one model on all X_train for validation evaluation.
            # 2. Only the loss on X_val is needed here, not valid p-values.
        ...
        try:
            ...
            if objective_type == "dsm_loss":
                ...
                # Contrast: how many standard deviations above null is the signal?
...
def run_hp_search(
    ...
    sampler = optuna.samplers.TPESampler(seed=sampler_seed, n_startup_trials=20)
    ...
    import multiprocessing
    if n_jobs is None:
        n_jobs = max(1, multiprocessing.cpu_count() - 1)  # Legacy behavior
    if n_jobs < 1:
...
def train_model(
    ...
    if output_dir:
        ...
        # Negate to align: positive mu_hat = excitatory, negative = inhibitory.
        np.savez(
            result_file,
...
def main():
    ...
    if args.mode == 'hp_search':
        ...
        # This supplies the default num_regimes when switching to regime_gated.
