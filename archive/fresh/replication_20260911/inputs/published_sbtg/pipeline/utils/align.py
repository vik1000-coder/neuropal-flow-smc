...
def assert_direction_convention(
    ...
    Args:
        ...
        A_truth: Reference adjacency matrix
...
def load_matrix_with_node_order(
    ...
    with np.load(input_path.with_suffix('.npz'), allow_pickle=False) as data:
        A = np.asarray(data['matrix'])
        node_order = [str(value) for value in data['node_order']]
...
def compute_in_degree(A: np.ndarray) -> np.ndarray:
    ...
    Under A[post, pre], in_degree is the row sum.
    """
...
def compute_out_degree(A: np.ndarray) -> np.ndarray:
    ...
    Under A[post, pre], out_degree is the column sum.
    """
