...
def ensure_aligned_atlas():
    ...
    Creates them if missing by aligning to the model neuron order.
    """
    ...
    # Load the model neuron order from prepared datasets.
    meta_file = DATASETS_DIR / "full_traces" / "standardization.json"
    if meta_file.exists():
        with open(meta_file, 'r') as f:
            model_neurons = json.load(f).get("node_order", [])
    else:
    ...
    print(f"  Aligning to {len(model_neurons)} neurons")
    ...
    # Find common
    common = [n for n in model_neurons if n in atlas_groups]
    ...
    for genotype, gkey in [("wild-type", "wt"), ("unc-31", "unc31")]:
        ...
        np.savez(
            ...
            q_eq=q_eq,
            dff=dff,
            neuron_order=np.asarray(common, dtype=str)
        )
...
def load_leifer_atlas_data() -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[List[str]]]:
    ...
    data = np.load(wt_file, allow_pickle=False)
    q = data['q']
