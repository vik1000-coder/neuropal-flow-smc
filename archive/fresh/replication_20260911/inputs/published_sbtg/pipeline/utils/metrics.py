...
RANDOM BASELINE:
===============
Fair random baselines must match the predicted edge density.

Expected F1 at predicted density ρ and reference prevalence π:
    E[precision] = π  (by construction: random guess)
    E[recall] = ρ  (fraction of positives hit by chance)
    E[F1] = 2 * π * ρ / (π + ρ)
...
def compute_binary_metrics(
    ...
    Args:
        y_true: Binary reference labels (n, n) or flattened
...
def compute_specificity_on_negatives(
    ...
    # True negatives = confirmed negatives predicted as negative.
    true_negatives = int(np.sum(pred_on_negatives == 0))
    # False positives = confirmed negatives predicted as positive.
...
def compute_random_baseline_f1(
    ...
    The correct formula for expected F1 at predicted density ρ and prevalence π:
        ...
        E[recall] = ρ (predicting fraction ρ hits ρ of positives on average)
        E[F1] = 2 * π * ρ / (π + ρ)
...
def compute_random_baseline_metrics(
    ...
    random_recall = rho  # Predicting fraction rho hits rho of positives.
...
def compute_metrics_at_density(
    ...
    different numbers of edges. Matching density permits comparison of
    precision/recall more fairly.
    
    Args:
        y_true: Binary reference labels (n, n)
...
def compute_weight_correlation(
    ...
    """
    Compute weight-level correlation metrics between predicted and reference weights.
    
    Args:
        predicted_weights: (n, n) predicted connectivity weights
        ground_truth_weights: (n, n) reference weights (e.g., synapse counts)
