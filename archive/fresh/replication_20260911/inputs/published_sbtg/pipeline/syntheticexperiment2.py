"""
SyntheticExperiment2 - large-scale synthetic benchmark.
...
def _run_external_baselines(
    ...
    if external_script is None:
        print("[EXTERNAL] Skipping: provide --external-script.", flush=True)
        return
    ext_script = external_script.expanduser().resolve()
    ...
    try:
        ...
        ext_csv = ext_script.parent / "evaluation_synthetic.csv"
...
def _write_complete_report(
    ...
    external_path = external_path_local
    ...
    if external_path.exists():
        ...
        lines.append(
            f"External baseline CSV not found at `{external_path_local}`.\n"
        )
...
def _augment_compact_report(
    ...
    if run_external_baselines:
        ...
        if ext_path.exists():
            ...
            lines.append(
                f"External baseline CSV not found at `{ext_path_local}`.\n"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run larger-scale synthetic SBTG experiments."
    )
    parser.add_argument(
        "--n-neurons",
        type=int,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to an optional external-baseline entrypoint.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
