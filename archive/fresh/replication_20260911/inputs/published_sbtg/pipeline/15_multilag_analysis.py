...
from utils.io import (
    load_structural_connectome as _load_structural_connectome,
    load_trace_collection,
)
...
def compute_pearson_baseline(X_list: List[np.ndarray], lag: int) -> np.ndarray:
    ...
    # Compute corr(x_{t+lag, j}, x_{t, i}) for each pair (j, i).
    ...
    for j in range(n): # Target
        for i in range(n): # Source
            ...
            for X in X_list:
                ...
                try:
                    # Test whether 'i' Granger-causes 'j'.
...
def load_neural_data(
    ...
    # Prefer the safe variable-length trace archive written by script 01.
    traces_file = dataset_dir / "traces.npz"
    if traces_file.exists():
        X_list = load_trace_collection(traces_file)
    else:
        ...
        if segment_files:
            ...
            if single_file.exists():
                X = np.load(single_file, allow_pickle=False)
                X_list = [X]
            else:
                raise FileNotFoundError(f"No trace files found in {dataset_dir}")
...
def load_cook_connectome(
    ...
    if A_gap_path.exists() and A_chem_path.exists():
        A_gap = np.load(A_gap_path, allow_pickle=False)
        A_chem = np.load(A_chem_path, allow_pickle=False)
    else:
...
def load_leifer_atlas(
    ...
    with np.load(wt_file, allow_pickle=False) as data:
        q = np.asarray(data['q']) if 'q' in data else None
        dff = np.asarray(data['dff']) if 'dff' in data else None
        neuron_names = [str(value) for value in data.get('neuron_order', [])]
...
def align_matrices(
    ...
    """
    Align predicted and reference matrices to common neurons.
...
def compute_auroc(
    ...
    Args:
        ...
        ground_truth: (n, n) reference matrix (binary or continuous)
...
def compute_spearman(
    ...
    """
    Compute Spearman correlation between predicted scores and reference weights.
    ...
    Args:
        scores: (n, n) predicted scores (continuous)
        ground_truth: (n, n) reference weights (continuous, e.g., synapse counts)
...
def evaluate_vs_leifer(
    ...
    # Create binary reference labels from q-values
...
def plot_auroc_vs_lag(
    ...
    ax.set_title(f'Approach {approach}: AUROC vs Time Lag\n(higher = better agreement with the reference)', fontsize=12)
    ...
    for period_name in PERIOD_NAMES:
        ...
        try:
            ...
            np.savez(
                ...
                **{f'p_value_lag{k}': v for k, v in result.p_values.items()},
                **{
                    f'significant_lag{k}': np.asarray(v, dtype=bool)
                    for k, v in result.significant.items()
                },
...
def plot_4period_comparison(period_results: dict, run_dir: Path, X_list: List[np.ndarray] = None):
    ...
    # Check for detailed evaluation data.
    has_detailed = False
    for period_name in available_periods:
...
def main():
    ...
    # Step 2: Load biological references
    print("\n" + "-"*60)
    print("[2/5] LOADING BIOLOGICAL REFERENCES")
    print("-"*60)
    ...
    if args.approach in ['A', 'all']:
        ...
        np.savez(
            ...
            **{f'p_value_lag{k}': v for k, v in result_A.p_values.items()},
            **{
                f'significant_lag{k}': np.asarray(v, dtype=bool)
                for k, v in result_A.significant.items()
            },
            neuron_names=neuron_names,
    ...
    if args.approach in ['B', 'all']:
        ...
        np.savez(
            ...
            **{f'p_value_lag{k}': v for k, v in result_B.p_values.items()},
            **{
                f'significant_lag{k}': np.asarray(v, dtype=bool)
                for k, v in result_B.significant.items()
            },
            neuron_names=neuron_names,
    ...
    if args.approach in ['C', 'all']:
        ...
        save_dict = {
            ...
            **{f'p_value_lag{k}': v for k, v in result_C.p_values.items()},
            **{
                f'significant_lag{k}': np.asarray(v, dtype=bool)
                for k, v in result_C.significant.items()
            },
            'neuron_names': neuron_names,
        ...
        # Store tuned HP configs in JSON rather than object-dtype NPZ fields.
        tuned_hp_configs = {}
        if args.tune_hp and hasattr(estimator_C, 'hp_configs'):
            for lag, config in estimator_C.hp_configs.items():
                tuned_hp_configs[str(lag)] = config.to_dict()
        
        np.savez(run_dir / 'result_C.npz', **save_dict)
        if tuned_hp_configs:
            with open(run_dir / 'hyperparameters_C.json', 'w') as handle:
                json.dump(tuned_hp_configs, handle, indent=2, sort_keys=True)
    ...
    # Prepare the Randi_Optogenetics_2023 functional reference if available
    leifer_gt = None
    if q_leifer is not None:
