...
Usage:
    python pipeline/16_celltype_analysis.py path/to/multilag_run
"""
...
def plot_clean_edge_density(data, output_dir):
    ...
    edge_counts = []
    for lag in lags:
        sig = data[f'significant_lag{lag}']
...
def plot_clean_type_interactions(data, type_indices, output_dir):
    ...
    for ax_idx, lag in enumerate(lags):
        ...
        mu_hat = data[f'mu_hat_lag{lag}']
        sig_mask = data[f'significant_lag{lag}']
        ...
        if fourperiod_dir.exists():
            ...
            for period in ['NOTHING', 'ON', 'SHOWING', 'OFF']:
                ...
                if result_file.exists():
                    ...
                    period_data = np.load(result_file, allow_pickle=False)
            ...
            for condition in ['ON', 'OFF']:
                ...
                if result_file.exists():
                    ...
                    cond_data = np.load(result_file, allow_pickle=False)
