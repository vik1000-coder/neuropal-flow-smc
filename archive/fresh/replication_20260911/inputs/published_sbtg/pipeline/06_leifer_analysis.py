...
def load_aligned_atlases() -> Tuple[Dict, Dict, List[str]]:
    ...
    wt_data = dict(np.load(wt_file, allow_pickle=False))
    unc31_data = dict(np.load(unc31_file, allow_pickle=False))
...
def load_cook_connectome(neurons: List[str]) -> np.ndarray:
    """Load the Cook anatomical connectome aligned to the model neuron order."""
    ...
    if struct_file.exists():
        connectome = np.load(struct_file, allow_pickle=False)
    elif chem_file.exists():
        chem = np.load(chem_file, allow_pickle=False)
        gap = (
            np.load(gap_file, allow_pickle=False)
            if gap_file.exists()
            else np.zeros_like(chem)
        )
...
def load_connectivity_matrices(neurons: List[str]) -> Dict[str, np.ndarray]:
    ...
    for stim in ["butanone", "pentanedione", "nacl", "full_traces"]:
        ...
        X_segments = load_trace_collection(x_file)
        valid_segments = []
        for s in X_segments:
...
def analyze_wt_only_edges(
    ...
    Test whether the evaluated methods preferentially score these extrasynaptic edges.
    """
    print("\n" + "=" * 60)
