...
def generate_phase_sbtg_data(force_regenerate: bool = False):
    ...
    X_segments = load_trace_collection(imputed_dir / 'traces.npz')
    ...
    # Phase definitions in seconds from the OH16230 acquisition protocol.
    ...
    for phase_name, (start_s, end_s) in phases.items():
        ...
        for worm_idx in range(n_worms):
            ...
            # Do not index past the end of the data.
        ...
        # Check whether enough data are available for SBTG training.
...
def create_fig6_predicted_vs_anatomical():
    ...
    # Panel B: Structural reference network
    ax2 = axes[1]
...
def create_fig8_aggregate_network():
    ...
    data = np.load(imputed_models[0], allow_pickle=False)
    sign_adj = data['sign_adj']
...
def create_fig11_weight_scatter():
    ...
    if leifer_file.exists():
        leifer_data = np.load(leifer_file, allow_pickle=False)
...
def main():
    ...
    if not args.quick:
        ...
        # Direct and transfer network figures
...
def create_fig14_mean_vs_volatility():
    ...
    vol_models = [f for f in models_dir.glob('*.npz') 
                  if 'volatility_adj' in np.load(f, allow_pickle=False).files]
