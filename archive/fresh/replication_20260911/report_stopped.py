...
from analyze import CHANNELS,CONTEXTS
...
![Matrix stability](analysis_stopped_20260916/{name}/figures/replication_stability.png)
![Lag profiles](analysis_stopped_20260916/{name}/figures/reference_lag_profiles.png)
![Lag matrices](analysis_stopped_20260916/{name}/figures/lag_effect_matrices.png)
![Support](analysis_stopped_20260916/{name}/figures/source_support.png)
...
![Factual rollout](analysis_stopped_20260916/{name}/figures/predictive_rollout.png)
![Particle convergence](analysis_stopped_20260916/{name}/figures/particle_convergence.png)
...
'''
    ...
    report=f'''# Flow / progressive bridge SMC: completed-primary analysis after user-directed stop

Primary analysis completed {now()}. Protocol identity: `{spec_id()}`.
...
'''
    (R/'REPORT_STOPPED.md').write_text(report)
    ...
    receipt={'status':'pass','meaning':'artifact completion and numerical schema validation, not a positive scientific verdict','spec_id':spec_id(),'completed_utc':now(),'primary_fits':30,'figure_count':len(plots),'report_sha256':sha(R/'REPORT_STOPPED.md'),'report_source_sha256':sha(Path(__file__)),'analysis_files':{str(p.relative_to(R)):sha(p) for p in required},'visual_review':'pending human/agent rendered inspection; not asserted by this numerical check'}
    atomic_json(R/'validation/final_stopped.json',receipt)
    print('REPORT_COMPLETE',R/'REPORT_STOPPED.md',flush=True)
