...
def fdr_control(pvals: np.ndarray, alpha: float, method: str = "bh") -> np.ndarray:
    ...
    # BUG FIX (audit): accept the 'benjamini-yekutieli' alias, not just 'by',
    # so the BY correction is not silently downgraded to BH.
    if method.lower() in {"by", "benjamini-yekutieli"}:
...
class SBTGStructuredVolatilityEstimator:
    ...
    def __init__(
        ...
        cross_fit_group_by_segment: bool = False,
        ...
        self.cross_fit_group_by_segment = bool(cross_fit_group_by_segment)
    ...
    def fit(self, X: Union[np.ndarray, List[np.ndarray]], skip_edge_tests: bool = False) -> SBTGVolatilityResult:
        ...
        if self.inference_mode == "cross_fit":
            ...
            if skip_edge_tests:
                 # BUG FIX (audit): SBTGVolatilityResult has no scores/p_values/q_values
                 # fields; construct a valid empty result with the real field names.
                 nn = self.n_neurons
                 result = SBTGVolatilityResult(
                     sign_adj=np.zeros((nn, nn), dtype=int),
                     volatility_adj=np.zeros((nn, nn), dtype=bool),
                     undirected_adj=None,
                     p_mean=np.ones((nn, nn)),
                     p_volatility=None,
                     mu_hat=np.zeros((nn, nn)),
                     volatility_stat=None,
                     W_param=np.zeros((nn, nn)),
                 )
            else:
    ...
    def _create_fold_assignments(
        ...
        unique_segments = np.unique(stim_ids)
        if self.cross_fit_group_by_segment:
            # Assign every window from one input sequence to the same fold.
            # In the NeuroPAL benchmark each input sequence is one worm, so
            # inner score cross-fitting remains strict at the animal level.
            for position, u in enumerate(rng.permutation(unique_segments)):
                fold_ids[stim_ids == u] = position % self.n_folds
            return fold_ids
