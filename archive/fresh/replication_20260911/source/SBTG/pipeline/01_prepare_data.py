...
def align_connectome(
    ...
    # BUG FIX (audit): the Cook spreadsheet lists row = PREsynaptic, col = POSTsynaptic,
    # so align_matrices_to_common_neurons produced A[pre, post]. The pipeline convention
    # (utils/align.py) is A[post, pre] = A[target, source]. Transpose chem to match.
    # Verified: e.g. ASH->AVA, AIY->RIA, AWC->AIY end up at [post, pre] after transpose.
    A_chem = A_chem.T.copy()
    # Gap junctions are electrical (bidirectional): ensure symmetric edge presence.
    A_gap = np.maximum(A_gap, A_gap.T)
...
def merge_head_tail_data(
    head_data: Dict,
    tail_data: Dict,
    simultaneous_pairing_table: Dict[str, str] | None = None,
) -> Dict:
    """
    ...
    # Fail closed. Array position is not evidence that two recordings are from
    # the same animal at the same time.
    if simultaneous_pairing_table is None:
        raise RuntimeError(
            "head/tail fusion requires an explicit simultaneous-recording pairing table"
        )
    head_ids = [str(value) for value in head_data["worm_ids"]]
    tail_ids = [str(value) for value in tail_data["worm_ids"]]
    if set(simultaneous_pairing_table) != set(head_ids):
        raise RuntimeError("pairing table does not cover the exact head recording IDs")
    paired_tail = [str(simultaneous_pairing_table[value]) for value in head_ids]
    if paired_tail != tail_ids or head_ids != tail_ids:
        raise RuntimeError("head/tail recording IDs or order do not prove simultaneous pairing")
    if not np.isclose(float(head_data["fps"]), float(tail_data["fps"])):
        raise RuntimeError("head/tail clocks disagree")
    if head_data["stim_names"] != tail_data["stim_names"]:
        raise RuntimeError("head/tail stimulus name schemas disagree")
    if not np.array_equal(head_data["stim_times"], tail_data["stim_times"]):
        raise RuntimeError("head/tail stimulus intervals disagree")
    for head_order, tail_order in zip(
        head_data["stims_per_worm"], tail_data["stims_per_worm"]
    ):
        if not np.array_equal(head_order, tail_order):
            raise RuntimeError("head/tail stimulus orders disagree")
...
def load_neuropal_data(
    ...
    if include_tail:
        ...
        data = merge_head_tail_data(
            head_data, tail_data, simultaneous_pairing_table
        )
    else:
        data = head_data
