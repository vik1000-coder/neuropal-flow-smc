...
def _output_path(
    ...
    return (
        run_dir / "responses"
        / f"{model_id}__smc__B{repair}__f{fold}__s{seed}.npz"
    )
    ...
    *,
    ...
    for worm_position, worm_idx in enumerate(heldout):
        ...
        for cut in episode_cuts(
            ...
            print(
                f"SMC_EPISODE fold={fold} seed={seed} worm={int(worm_idx)} "
                f"phase={cut.phase} event={cut.event}",
                flush=True,
            )
    ...
    np.savez_compressed(
        ...
        estimator=np.asarray("bootstrap_smc_ess"),
...
def main() -> None:
    ...
    parser.add_argument("--horizons", nargs="+", type=int, default=[1, 2, 4, 8, 16, 24, 32, 40])
    parser.add_argument("--n-particles", type=int, default=64)
    parser.add_argument("--resample-ess-fraction", type=float, default=0.50)
    ...
    manifest = {
        ...
        "estimator": "bootstrap SMC with incremental anchor, terminal clamp, ESS-triggered systematic resampling, terminal resampling, and free rollout",
        "contrast_coupling": "paired common random numbers and paired systematic-resampling offsets for low/high targets; marginal SMC targets unchanged",
    ...
    for fold in args.folds:
        for seed in args.seeds:
            print(f"SMC_START fold={fold} seed={seed}", flush=True)
            try:
                ...
                print(f"SMC_FAILED {record}", flush=True)
            records.append(record)
            _write_state(run_dir, manifest, records)
            print(f"SMC_DONE {record}", flush=True)
    manifest["status"] = (
        "complete" if all(record["status"] in {"ok", "skipped"} for record in records) else "partial"
    )
