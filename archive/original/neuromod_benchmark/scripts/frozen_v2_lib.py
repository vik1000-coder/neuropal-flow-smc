#!/usr/bin/env python3
"""Freeze-gate and sequential-execution helpers for the frozen-v2 benchmark.
...
MAX_THREADS = 2
REQUIRED_NICE = 19
FINAL_CONFIG_DIR = PACKAGE_ROOT / "configs" / "frozen_v2"
DRAFT_CONFIG_DIR = PACKAGE_ROOT / "configs" / "frozen_v2_draft"
FROZEN_OUTPUT_ROOT = PACKAGE_ROOT / "outputs" / "frozen_v2"
...
class FrozenV2Error(RuntimeError):
...
def resolve_config_dir(requested: str | Path | None = None) -> tuple[Path, str]:
    ...
    if not selected.is_dir():
        raise FrozenV2Error(f"configuration directory does not exist: {selected}")
...
def config_paths(config_dir: Path) -> tuple[Path, ...]:
    ...
    if not paths:
        raise FrozenV2Error(f"no suite configurations found in {config_dir}")
    stems = [path.stem for path in paths]
    if len(stems) != len(set(stems)):
        raise FrozenV2Error("configuration stems must be unique across YAML/JSON files")
    return paths
...
def classify_config(path: Path) -> str:
    ...
    if not isinstance(raw, Mapping):
        raise FrozenV2Error(f"{path.name}: YAML must contain a mapping")
    if "max_lags" in raw:
        return "memory"
...
def _resource_dict(resource: ResourceBudget) -> dict[str, Any]:
    ...
    if resource.max_parallel_jobs != 1:
        raise FrozenV2Error("frozen execution requires max_parallel_jobs=1")
    if resource.nice != REQUIRED_NICE:
        raise FrozenV2Error(f"frozen execution requires nice={REQUIRED_NICE}")
...
def _validate_scenario_ids(scenarios: Sequence[ScenarioSpec]) -> set[str]:
    ...
    if not all(isinstance(identifier, str) and identifier for identifier in ids):
        raise FrozenV2Error("scenario IDs must be non-empty strings")
    if len(ids) != len(set(ids)):
        raise FrozenV2Error("scenario IDs must be unique within a suite")
    return set(ids)
...
def _method_plans(
    ...
    *,
    ...
    for method_index, method in enumerate(methods):
        ...
        if not grid:
            raise FrozenV2Error(f"method {method.name!r} has an empty parameter grid")
        eligible_seeds = (
        ...
        if not eligible_seeds:
            raise FrozenV2Error(f"method {method.name!r} has no eligible suite seed")
        capabilities = []
        classes = []
        score_domains = []
        for params in grid:
...
def _plan_main(
    ...
    *,
    ...
    if case_records < 1:
        raise FrozenV2Error(f"suite {suite.name!r} contains no executable cases")
    empty_methods = [
        name for name, item in method_plans.items() if int(item["context_count"]) == 0
    ]
    if empty_methods:
        raise FrozenV2Error(f"methods have no executable context: {empty_methods}")
    for item in method_plans.values():
...
def _plan_memory(path: Path) -> SuitePlan:
    ...
    if empty:
        raise FrozenV2Error(f"methods have no executable memory context: {empty}")
    for item in method_plans.values():
        item["result_record_count"] = int(item["context_count"])
    candidates, fits = _counts_from_method_contexts(method_plans)
    return SuitePlan(
...
def _plan_robustness(path: Path) -> SuitePlan:
    ...
    empty = [name for name, item in method_plans.items() if not item["context_count"]]
    if empty:
        raise FrozenV2Error(f"methods have no executable robustness context: {empty}")
    for item in method_plans.values():
...
def _positive_int(raw: Mapping[str, Any], key: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < 1:
        raise FrozenV2Error(f"changepoint {key} must be a positive integer")
    return value
...
def _validate_changepoint_options(config: MechanisticConfig, options: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise FrozenV2Error(f"invalid changepoint benchmark arguments: {error}") from error
    methods = tuple(options.get("methods", ()))
    if not methods or len(methods) != len(set(methods)):
        raise FrozenV2Error("changepoint methods must be non-empty and unique")
    unknown = set(methods) - set(METHOD_CLAIM_AXIS)
    if unknown:
        raise FrozenV2Error(f"unknown changepoint detectors: {sorted(unknown)}")
    ...
    if not 0 < alpha < 1 or not 0 < boundary_fraction < 1 or not 0 < confidence < 1:
        raise FrozenV2Error("changepoint alpha, boundary_fraction, and confidence must lie in (0,1)")
    ...
    if not thresholds or any(value <= 1 for value in thresholds):
        raise FrozenV2Error("changepoint tail thresholds must exceed one")
    if thresholds != tuple(sorted(set(thresholds))):
        raise FrozenV2Error("changepoint tail thresholds must be strictly increasing")
    if options.get("variance_estimator", "block_median") not in {"sample", "block_median"}:
        raise FrozenV2Error("invalid changepoint variance estimator")
    if int(options.get("variance_block_size", 32)) < 8:
        raise FrozenV2Error("changepoint variance block size must be at least eight")
    if options.get("variance_channel_aggregation", "max") not in {"max", "sum"}:
        raise FrozenV2Error("invalid changepoint variance channel aggregation")
    if (
        ...
        raise FrozenV2Error("changepoint boundary overlaps the frozen reference prefix")
...
def _plan_changepoint(path: Path) -> SuitePlan:
    ...
    if not isinstance(strengths_raw, Mapping):
        raise FrozenV2Error("changepoint post_change_strengths must be a mapping")
    strengths = {
    ...
    if set(strengths) - {"mean", "dispersion", "matched_tail"}:
        raise FrozenV2Error("changepoint post_change_strengths has an unknown scenario")
...
def _validate_output_isolation(plans: Sequence[SuitePlan]) -> None:
    ...
    for plan, output in zip(plans, outputs):
        ...
        if output == state or _is_relative_to(output, state):
            raise FrozenV2Error(f"suite output overlaps orchestrator state: {output}")
    if len(outputs) != len(set(outputs)):
        raise FrozenV2Error("suite output directories must be unique")
    for index, left in enumerate(outputs):
        for right in outputs[index + 1 :]:
            if _is_relative_to(left, right) or _is_relative_to(right, left):
                raise FrozenV2Error(f"nested suite outputs are forbidden: {left} and {right}")
...
def _validate_existing_output(
    ...
    provenance_path = output / "frozen_v2_provenance.json"
    if provenance_path.exists():
        ...
        if provenance.get("config_sha256") != plan.config_sha256:
            raise FrozenV2Error(f"{plan.identifier}: existing output has another config hash")
    ...
    if not manifest_path.exists():
        raise FrozenV2Error(f"{plan.identifier}: nonempty output lacks run_manifest.json")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("suite_config_digest") != plan.suite_config_digest:
        raise FrozenV2Error(f"{plan.identifier}: output suite-config digest differs")
    ...
    if manifest.get("status") not in allowed_resume_statuses:
        raise FrozenV2Error(
            ...
            "is not resumable into frozen-v2"
        )
    for result_path in sorted((output / "results").glob("*.json")):
        ...
        if record.get("status") != "ok":
            raise FrozenV2Error(
                ...
                f"{record.get('status')!r}; failed/partial cases cannot enter frozen-v2"
            )
...
def _environment_identity(environment: Mapping[str, Any]) -> dict[str, str]:
    ...
    if missing:
        raise FrozenV2Error(f"could not freeze required environment digests: {missing}")
...
def _validate_external_causal_environment(
    ...
    if not isinstance(packages, Mapping):
        raise FrozenV2Error("external causal methods require the isolated .causal_venv")
    missing = sorted(package for package in required if package not in packages)
    if missing:
        raise FrozenV2Error(f"isolated causal environment is missing packages: {missing}")
    ...
    *,
    ...
    if mode in {"final", "draft"}:
        ...
        if observed != expected:
            raise FrozenV2Error(
                "frozen-v2 suite set differs from the preregistered 15 suites; "
...
def assert_baseline_immutable(preflight_manifest: Mapping[str, Any]) -> None:
    ...
    if current_hashes != expected_hashes:
        raise FrozenV2Error("frozen configuration hashes have changed since preflight")
...
def load_preflight(path: Path = DEFAULT_PREFLIGHT_PATH) -> dict[str, Any]:
    if not path.is_file():
        raise FrozenV2Error(f"successful preflight manifest not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != SCHEMA_VERSION or payload.get("status") != "passed":
        raise FrozenV2Error("preflight manifest is not a successful supported manifest")
    if not payload.get("policy", {}).get("response_truth_preflight"):
        raise FrozenV2Error("frozen execution requires response-truth preflight")
...
def command_for_plan(plan: SuitePlan) -> list[str]:
    ...
    raise FrozenV2Error(f"unsupported plan kind {plan.kind!r}")
...
def _execution_environment(max_threads: int) -> dict[str, str]:
    if not 1 <= max_threads <= MAX_THREADS:
        raise FrozenV2Error(f"invalid execution thread ceiling {max_threads}")
...
def _run_monitored(
    ...
    *,
    ...
    if heartbeat_seconds < 5 or heartbeat_seconds > 60:
        raise FrozenV2Error("heartbeat_seconds must lie in [5,60]")
    ...
    if free_gib < disk_floor:
        raise FrozenV2Error(f"free disk {free_gib:.2f} GiB is below {disk_floor:.2f} GiB")
    ...
    if nice is None:
        raise FrozenV2Error("the POSIX nice executable is required for frozen runs")
...
def _validate_changepoint_payload(
    ...
    if not isinstance(calibrations, Mapping) or set(calibrations) != set(expected_methods):
        raise FrozenV2Error(f"{plan.identifier}: calibration method set differs from config")
    for method, calibration in calibrations.items():
        if not isinstance(calibration, Mapping):
            raise FrozenV2Error(f"{plan.identifier}: invalid calibration for {method}")
    ...
    for row in detections:
        if not isinstance(row, Mapping):
            raise FrozenV2Error(f"{plan.identifier}: a detection row is not a mapping")
        identity = (str(row.get("series_id")), str(row.get("method")))
        if identity in identities:
            raise FrozenV2Error(f"{plan.identifier}: duplicate detection {identity}")
        identities.add(identity)
        if row.get("method") not in expected_methods:
            raise FrozenV2Error(f"{plan.identifier}: detection uses an unknown method")
        ...
        if not 0 <= float(row["p_value"]) <= 1:
            raise FrozenV2Error(f"{plan.identifier}: detection p-value is outside [0,1]")
        if isinstance(row.get("tau_hat"), bool) or not isinstance(row.get("tau_hat"), int):
            raise FrozenV2Error(f"{plan.identifier}: detection tau_hat is not an integer")
        if not isinstance(row.get("called"), bool):
            raise FrozenV2Error(f"{plan.identifier}: detection called flag is invalid")
        if row.get("scenario") == "null":
            if not isinstance(row.get("false_positive"), bool):
                raise FrozenV2Error(f"{plan.identifier}: null detection lacks an FPR flag")
        else:
    ...
    if not payload.get("detection_summary") or not payload.get("attribution_summary"):
        raise FrozenV2Error(f"{plan.identifier}: changepoint summaries are missing")
    return {
        "status": "complete",
    ...
    *,
    ...
    if plan.kind == "changepoint":
        ...
        if not result_path.is_file():
            raise FrozenV2Error(f"{plan.identifier}: missing changepoint result")
        payload = json.loads(result_path.read_text(encoding="utf-8"))
        if payload.get("schema_version") != 1:
            raise FrozenV2Error(f"{plan.identifier}: unsupported result schema")
        raw_config = json.loads(Path(plan.config_path).read_text(encoding="utf-8"))
        if payload.get("config") != raw_config:
            raise FrozenV2Error(f"{plan.identifier}: result embeds a different config")
        return _validate_changepoint_payload(plan, payload, raw_config)
    manifest_path = output / "run_manifest.json"
    if not manifest_path.is_file():
        raise FrozenV2Error(f"{plan.identifier}: missing run manifest")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("suite_config_digest") != plan.suite_config_digest:
        raise FrozenV2Error(f"{plan.identifier}: suite-config digest mismatch")
    observed_source = manifest.get("environment", {}).get("benchmark_source_digest")
    if observed_source != source_digest:
        raise FrozenV2Error(f"{plan.identifier}: benchmark-source digest mismatch")
    ...
    if (
        ...
        raise FrozenV2Error(f"{plan.identifier}: SID source digest mismatch")
    if (
    ...
    if any(not isinstance(case_id, str) or not case_id for case_id in case_ids):
        raise FrozenV2Error(f"{plan.identifier}: a result has a missing case ID")
    if len(case_ids) != len(set(case_ids)):
        raise FrozenV2Error(f"{plan.identifier}: duplicate case IDs are present")
    ...
    if require_report:
        ...
        if not validation_path.is_file() or not report_path.is_file():
            raise FrozenV2Error(f"{plan.identifier}: report/validation artifacts are missing")
...
def _write_provenance(
    ...
    *,
    ...
    atomic_json(
        Path(plan.output_dir) / "frozen_v2_provenance.json",
        {
            "schema_version": SCHEMA_VERSION,
            "suite": plan.identifier,
...
class ExecutionLock:
    ...
    def __enter__(self) -> "ExecutionLock":
        ...
        try:
            ...
            self.handle.close()
            raise FrozenV2Error("another frozen-v2 orchestrator holds the execution lock") from error
        self.handle.seek(0)
    ...
    *,
    ...
    for key in (
        ...
        if state.get(key) != preflight_manifest.get(key):
            raise FrozenV2Error(f"execution state has another frozen {key}")
    if state.get("execution_order") != [plan.identifier for plan in plans]:
        raise FrozenV2Error("execution order differs from the frozen preflight")
    for plan in plans:
        suite_state = state.get("suites", {}).get(plan.identifier, {})
        if suite_state.get("config_sha256") != plan.config_sha256:
            raise FrozenV2Error(f"{plan.identifier}: execution-state config hash differs")
    ...
    *,
    ...
    if preflight_manifest.get("config_mode") != "final" and not allow_draft_run:
        raise FrozenV2Error(
            ...
            "configs/frozen_v2 and rerun preflight"
        )
...
def build_execution_summary(
    *,
    ...
    preflight_manifest = load_preflight(preflight_path)
    if not state_path.is_file():
        raise FrozenV2Error(f"execution manifest does not exist: {state_path}")
    state = json.loads(state_path.read_text(encoding="utf-8"))
    if state.get("status") != "complete":
        raise FrozenV2Error(f"execution status is not complete: {state.get('status')!r}")
    if state.get("preflight_id") != preflight_manifest.get("preflight_id"):
        raise FrozenV2Error("execution and preflight identities differ")
    suites = []
    plans = tuple(_plan_from_dict(raw) for raw in preflight_manifest["suites"])
    for plan in plans:
        suite_state = state.get("suites", {}).get(plan.identifier, {})
        if suite_state.get("status") != "complete":
            raise FrozenV2Error(f"{plan.identifier}: orchestrator status is not complete")
        verification = verify_suite_completion(
            plan,
            source_digest=preflight_manifest["source_tree_digest"],
        ...
        provenance_path = Path(plan.output_dir) / "frozen_v2_provenance.json"
        if not provenance_path.is_file():
            raise FrozenV2Error(f"{plan.identifier}: missing frozen provenance")
        provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
        if provenance.get("preflight_id") != preflight_manifest["preflight_id"]:
            raise FrozenV2Error(f"{plan.identifier}: provenance preflight ID differs")
        for key in (
            "source_tree_digest",
            "sid_neuromod_source_digest",
            "execution_environment_digest",
