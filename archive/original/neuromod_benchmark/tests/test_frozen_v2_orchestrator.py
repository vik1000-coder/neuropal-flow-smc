...
from frozen_v2_lib import (  # noqa: E402
    FrozenV2Error,
    SuitePlan,
    plan_config,
...
schema_version: "1"
name: counted
output_dir: outputs/frozen_v2/test_counted
history_lags: [1]
horizons: [1, 2]
...
output_dir: outputs/frozen_v2/test_duplicate_views
history_lags: [1]
horizons: [1]
views: [latent, calcium]
seeds: [3]
...
"""
    ...
    with pytest.raises(FrozenV2Error, match="overlaps one execution context"):
...
output_dir: outputs/frozen_v2/test_corruption_tune
history_lags: [1]
horizons: [1]
views: [latent]
seeds: [3]
...
""".strip()
    ...
    with pytest.raises(FrozenV2Error, match="fixed aliases"):
        plan_config(config)
...
def test_completion_rejects_null_primary_score_even_when_record_says_ok(
    ...
    with pytest.raises(FrozenV2Error, match="primary score"):
...
def test_main_score_model_requires_finite_fixed_reference_ladder_mean(
    ...
    with pytest.raises(FrozenV2Error, match="exactly one fixed-reference ladder"):
        verify_suite_completion(plan, source_digest="source", require_report=False)
    path.write_text(
        json.dumps(
            {
...
def test_official_causal_worker_version_must_match_frozen_environment(
    ...
    result_path.write_text(json.dumps(record), encoding="utf-8")
    with pytest.raises(FrozenV2Error, match="does not match frozen environment"):
