...
def main():
    ...
    for label,frame,value,extra in [('predictive_energy',ps,'energy',None),('tail_probability_error',ps,'tail_probability_abs_error',None),('common_effect_error',es[es.event=='common'],'qualified_absolute_error',None),('rare_effect_error',es[es.event=='rare'],'qualified_absolute_error',None),('reference_qualification',es,'qualified_fraction',None),('mean_derivative_error',ds[ds.step==.1],'absolute_error',None)]:
        ...
        for ax,region in zip(axes,['central','boundary','extrapolated']):
            table=frame[frame.region==region].pivot_table(index='family',columns='law',values=value,aggfunc='mean').reindex(sorted(ps.family.unique()))
            table.plot.bar(ax=ax,legend=False);ax.set_title(region);ax.set_ylabel(value.replace('_',' '));ax.tick_params(axis='x',rotation=60)
        handles,labels=axes[0].get_legend_handles_labels();fig.legend(handles,labels,loc='outside upper center',ncol=4)
        fig.suptitle(label.replace('_',' ')+' — synthetic benchmark; missing qualified cells are not zero',y=1.09)
